jQuery(function($){var path=window.location.href;$('.portal-nav-item a').each(function(){if(this.href===path){$(this).addClass('active');}});});$(document).ready(function(){$('#BrowserRequest24hDataTable').DataTable({"language":{"emptyTable":"Couldn't retrieve country traffic data due to low/no traffic"}})});$(document).ready(function(){$('#HourlyRequestDataTable').DataTable({pageLength:24,paging:false,searching:false,})});$(document).ready(function(){$('#BrowserMap24hDataTable').DataTable({"language":{"emptyTable":"Couldn't retrieve browser traffic data due to low/no traffic"}})});$(document).ready(function(){$('#FirewallEventsDataTable').DataTable({"language":{"emptyTable":"There were no threats detected by the Cloudflare firewall"}})});;$(document).ready(function(){$('.toast-notifications').toast('show');});AOS.init({disable:false,startEvent:'DOMContentLoaded',initClassName:'aos-init',animatedClassName:'aos-animate',useClassNames:false,disableMutationObserver:false,debounceDelay:50,throttleDelay:99,offset:0,delay:0,duration:400,easing:'ease',once:false,mirror:false,anchorPlacement:'center-bottom',});function isOnScreen(elem){if(elem.length==0){return;}
...
window.location=returnToURL;return false};Sentry.init({dsn:"",integrations:[new Sentry.Integrations.BrowserTracing()],tracesSampleRate:1.0,release:"1.0.0",environment:"development",});;